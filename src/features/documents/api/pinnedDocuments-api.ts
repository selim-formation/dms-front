...
import type { PinnedDocumentData, PinnedDocumentsListParams } from '../types/pinned.types';
...
export async function getPinnedDocuments(
    ...
    try {
        log.info('Fetching all pinned documents from API', { tenant, params });
...
/**
 * Fetch a single pinned document from API
 */
export async function getPinnedDocument(
    ...
    try {
        log.info('Fetching pinned document from API', { tenant, pinnedDocumentId });
        return await pinnedDocumentsApiService.fetchPinnedDocument(tenant, pinnedDocumentId);
    } catch (error) {
...
/**
 * Reorder a pinned document from API
 */
export async function updatePinnedDocumentOrder(
    tenant: string,
    pinnedDocumentId: number,
    order: number
): Promise<PinnedDocumentData> {
    try {
        log.info('Reordering pinned document from API', { tenant, pinnedDocumentId, order });
        const pinned = await pinnedDocumentsApiService.updatePinnedDocumentOrder(
            tenant,
            pinnedDocumentId,
            order
        );
