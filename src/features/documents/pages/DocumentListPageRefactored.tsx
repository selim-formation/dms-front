...
export default function DocumentListPageRefactored(): React.ReactElement {
    ...
    const { filteredDocuments: allViewFiltered } = useDocumentFiltering({
        documents: (allDocumentsData?.data || []) as unknown as UIDocument[],
        searchQuery,
    ...
    const { filteredDocuments: typeViewFiltered } = useDocumentFiltering({
        documents: [
            ...typeViewDocs.establishment.renewal,
            ...typeViewDocs.establishment.oneTime,
            ...typeViewDocs.operational.renewal,
            ...typeViewDocs.operational.oneTime,
        ] as unknown as UIDocument[],
        searchQuery,
        filters: selectedFilters,
        enabled: viewMode === 'byType',
    });
    ...
    const { filteredDocuments: deptViewFiltered } = useDocumentFiltering({
        documents: [...deptViewDocs.renewal, ...deptViewDocs.oneTime] as unknown as UIDocument[],
        searchQuery,
        filters: selectedFilters,
        enabled: viewMode === 'byDepartment',
    });
                    ...
                    <div className="flex items-center justify-center h-96">
                        <div className="text-center max-w-md">
                            <div className="mb-6 inline-block p-6 bg-red-50 rounded-lg border border-red-200">
                                ...
                                <p className="text-red-600 text-sm mb-4 font-mono break-words">{errorMessage}</p>
    ...
    const resultCount = viewMode === 'byType' ? typeViewFiltered.length : viewMode === 'byDepartment' ? deptViewFiltered.length : allViewFiltered.length;
