...
export default function DocumentCardGrid({
  ...
  const categoryLabel = translateCategoryName(category, i18n.language);
  const {
    isPinned,
    isPending: isPinPending,
    toggle: togglePin,
  } = useTogglePin(documentId);
  const {
    isFavorited,
  ...
  const { download: downloadDefault, isDownloading: isDownloadingDefault } =
    useDownloadDocument();
  const handleDownload =
    onDownload ?? (() => downloadDefault({ documentId, filename: name }));
  const effectiveIsDownloading = onDownload
    ? isDownloading
    ...
    >
      ...
      <div className="relative h-32 bg-gradient-to-br from-muted to-muted flex items-center justify-center border-b border-border">
        ...
        <div className="absolute top-3 end-3 flex items-center gap-1 bg-card/80 rounded-lg backdrop-blur-sm">
          ...
          <DocumentIconToggleButton
            ...
            onClick={togglePin}
            label={
              isPinned ? t("common:actions.unpin") : t("common:actions.pin")
            }
          />
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="pt-1 space-y-1.5 border-t border-border">
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="shrink-0 text-xs text-muted-foreground">
              {t("documentCardGrid.statusLabel")}
            </span>
            <span
              title={statusLabel ?? statusLabels[status]}
              className={`min-w-0 max-w-[65%] truncate px-2 py-0.5 text-xs font-semibold rounded-lg text-center ${
                status === "Expired"
        ...
        <div className="pt-2 flex gap-2">
          <button
            onClick={
              onView ?? (() => navigate({ to: `/${tenant}/documents/${id}` }))
            }
