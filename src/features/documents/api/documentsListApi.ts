...
export class DocumentsListApiService {
  ...
  public async fetchDocumentsByTypes(
    tenant: string,
  ): Promise<DocumentsByTypeItem[]> {
    try {
  ...
  public async fetchDocumentsByDepartments(
    tenant: string,
  ): Promise<DocumentsByDepartmentItem[]> {
    try {
      const url = buildApiUrl(this.endpoints.byDepartments, { tenant });
      const response =
        await this.client.get<DocumentsByDepartmentApiResponse>(url);
      return response.data.data ?? [];
    } catch (error) {
  ...
  public async fetchCategorized(
    ...
    try {
      ...
      const response = await this.client.post<CategorizedApiResponse>(
        url,
        filters,
      );
  ...
  public async searchDocuments(
    ...
    try {
      ...
      const response = await this.client.post<DocumentSearchApiResponse>(
        url,
        filters,
      );
