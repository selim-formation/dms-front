...
interface UsePinnedDocumentsOptions extends PinnedDocumentsListParams {
    enabled?: boolean;
}
...
export function usePinnedDocuments(
    ...
    const { enabled: enabledProp = true, number_of_documents } = options;
    const tenant = useTenantId();

    const enabled = enabledProp && !!tenant;
    const params: PinnedDocumentsListParams = { number_of_documents };

    const query = useQuery({
        queryKey: tenant
            ? [...pinnedDocumentsKeys.list(tenant), params]
        ...
        queryFn: async () => {
            ...
            log.info('Fetching pinned documents', { tenant, params });
            return getPinnedDocuments(tenant, params);
        },
        enabled,
        ...QUERY_CONFIG,
    });
